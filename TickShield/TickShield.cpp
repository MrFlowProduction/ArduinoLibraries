...
void TickShield::sensorInit()
{
  pinMode(TEMPERATURE_SENSOR, INPUT);
  pinMode(LIGHT_SENSOR, INPUT);
}
void TickShield::buzzerInit()
{
	pinMode(ALARM_BUZZER, OUTPUT);
  	digitalWrite(ALARM_BUZZER,LOW);
}
void TickShield::setLed(unsigned char led_status, int pinLED)
{
	digitalWrite(pinLED, led_status);
}
/********************************/
void TickShield::turnOffLED()
{
...
/*******************************/
void TickShield::runLED(byte speed, byte direction)
{
	turnOffLED();
...
void TickShield::processSystemStatus()
{
...
inline void TickShield::modifyAlarmFlag()
{
	turnOffLED();
...
void TickShield::adjustClock(uint8_t hand)
{
	turnOffLED();
	setLed(HIGH,LED_CLOCK_ADJUST);
	if(hand == HOUR)
...
void TickShield::adjustAlarm(uint8_t hand)
{
	turnOffLED();
	setLed(HIGH,LED_ALARM_ADJUST);
	if(hand == HOUR)
...
/***********************************/
void TickShield::adjustBrightness()
{
	turnOffLED();
	setLed(HIGH,LED_BRIGHT_ADJUST);
	if(key_pin_pressed == KEY_DOWN)
...
/*Return:	  void																												*/
void TickShield::displayTemperature(int8_t temperature)
{
  int8_t temp[4];
  if(temperature < 0)
	{
		temp[0] = INDEX_NEGATIVE_SIGH;
...
float TickShield::getLightIntensity()
{
...
uint8_t TickShield::getLightLevel()
{
	uint16_t resistance;
	uint8_t light_level;
...
void TickShield::ringOn()
{
	digitalWrite(ALARM_BUZZER, HIGH);
}
void TickShield::ringOff()
{
	digitalWrite(ALARM_BUZZER, LOW);
}
void TickShield::alarming()
{
	if(alarm_on_off)ringOn();
	else ringOff();
}
void TickShield::turnOffAlarm()
{
	ringOff();
	flag_alarm_over = 1;
}
boolean TickShield::isAlarmEnable()
{
	if(compareWithAlarm())
	{
		if(!flag_alarm_over)
...
/*******************************/
boolean TickShield::compareWithAlarm()
{
...
/*Return:   void																							   */
void TickShield::writeToAdjustArea()
{
	g_hour_temp       = g_hour;
...
/*Return:   void																							   */
void TickShield::writeToNormalArea()
{
	g_hour       = g_hour_temp;
	g_minute     = g_minute_temp;
	g_brightness = g_brightness_temp;
...
void TickShield::display(int8_t DispData [])
{
	tm1636.display(DispData);
}

void TickShield::clearTime(uint8_t hand)
{
	unsigned char time[4];
	if(hand == HOUR)
	{
...
byte TickShield::decToBcd(byte val)
{
	return ( (val/10*16) + (val%10) );
}

// Convert binary coded decimal to normal decimal numbers
byte TickShield::bcdToDec(byte val)
{
	return ( (val/16*10) + (val%16) );
}

boolean TickShield::isAdjustingTime()
{
